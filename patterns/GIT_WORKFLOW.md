...
- Treat `gi РїСѓР»`, `gi pull`, and `РіРё РїСѓР»` as explicit requests to update the
  current branch from its configured upstream.
- `gi РєРѕРјРјРёС‚` means commit only.
- `gi РїСѓС€` means commit the scoped current changes, then push the current branch.
- `gi РєРѕРјРјРёС‚ РїСѓС€` means the same as `gi РїСѓС€`.
- `gi С‚РѕР»СЊРєРѕ РїСѓС€` means push existing local commits only; do not create a new
...
- Do not reinterpret `gi РїСѓС€` as a raw `git push`, a retry of a previous
  ...
  `gi С‚РѕР»СЊРєРѕ РїСѓС€`.
- `gi РїСѓР»` means fetch and pull the current branch only; do not switch branches,
  rewrite history, or pull from a different remote unless the user explicitly
  asks.
- Exception: a successful `gi РѕР±РЅРѕРІРёС‚СЊ` / `gi РѕР±РЅРѕРІРёСЃСЊ` is an explicit request to
...
- In a shared instruction-library repository, a user request to add or accept a
  ...
  then run the `gi РѕР±РЅРѕРІРёС‚СЊ` update flow when accepted instruction-kit
...
Before any `gi РєРѕРјРјРёС‚`, `gi РїСѓС€`, `gi РєРѕРјРјРёС‚ РїСѓС€`, or `gi С‚РѕР»СЊРєРѕ РїСѓС€` action:
...
For `gi РїСѓС€` and `gi РєРѕРјРјРёС‚ РїСѓС€`:
...
Before any `gi РїСѓР»`, `gi pull`, or `РіРё РїСѓР»` action:
...
`gi СЏР·С‹Рє РєРѕРјРјРёС‚Р°` forms as the short shared-instruction command forms for this
workflow.

Do not use commit-language commands to change the agent's user-facing working
language. That is configured separately with `gi system language`, `gi СЃРёСЃС‚РµРј
СЏР·С‹Рє`, or `РіРё СЃРёСЃС‚РµРј СЏР·С‹Рє` in `tools/project-memory/system-preferences.json`.
