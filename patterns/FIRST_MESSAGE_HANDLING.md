...
Also treat `gi init <source>`, `init <source>`, `РёРЅРёС‚ <source>`, and
`РёРЅРёС†РёР°Р»РёР·РёСЂСѓР№ <source>` as an instruction bootstrap when the source points to
...
Treat `РёРЅРёС‚ РїСЂР°РІРёР»Р° <source>` the same way when `<source>` points to
...
- `gi РѕР±РЅРѕРІРёСЃСЊ`: check or apply accepted instruction-kit updates.
...
- `РёРЅРёС‚ https://github.com/Dimosfil/general-instructions.git` or
...
- `РёРЅРёС‚ <path-to-general-instructions>`: bootstrap/startup from the existing
  shared-instruction checkout; never run `git init`.
- `РёРЅРёС‚ РїСЂР°РІРёР»Р° <path-to-general-instructions>`: load instruction rules from the
  existing shared-instruction checkout; never run `git init`.
- `gi commit language: Russian`, `gi РєРѕРјРјРёС‚ СЏР·С‹Рє: Russian`, or
  `РіРё РєРѕРјРјРёС‚ СЏР·С‹Рє: Russian`: update commit-message language preferences.
- `gi СЏР·С‹Рє РєРѕРјРјРёС‚Р°: Russian`: older alias for commit-message language
  preferences.
- `gi commit language: English only`: keep English as the only commit-message
  language.
- `gi system language: Russian`, `gi СЃРёСЃС‚РµРј СЏР·С‹Рє: Russian`, or
  `РіРё СЃРёСЃС‚РµРј СЏР·С‹Рє: Russian`: update the agent's user-facing working language
  for this project.
- `gi СЃР°РјРјР°СЂРё` or `gi summary`: write a handoff summary file under
  `tools/summary/`.
- `gi СЃС‚Р°СЂС‚` or `gi restore`: restore project context from `AGENTS.md`, the
...
- `gi local sprint`, `gi sprint local`, `gi Р»РѕРєР°Р»СЊРЅС‹Р№ СЃРїСЂРёРЅС‚`,
  `gi СЃРїСЂРёРЅС‚ Р»РѕРєР°Р»СЊРЅРѕ`, or equivalent explicitly local sprint wording: run a
...
- `gi РіРёС‚-РѕР±Р·РѕСЂ` or `gi git summary`: summarize the latest git commit in the
  current project without printing a full diff.
- `gi С‚РµСЃС‚-РїР»Р°РЅ` or `gi test plan`: build a project-aware test plan for a new
  feature, bug fix, or release check.
- `gi РєРѕРјРјРёС‚`: commit scoped current changes without pushing.
- `gi РїСѓС€` or `gi РєРѕРјРјРёС‚ РїСѓС€`: commit scoped current changes, then push the
  current branch.
- `gi С‚РѕР»СЊРєРѕ РїСѓС€`: push existing local commits without creating a new commit.
- `gi РїСѓР»`, `gi pull`, or `РіРё РїСѓР»`: fetch and pull the current branch, resolving
